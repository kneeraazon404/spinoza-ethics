...
const SpinozaEthicsDiagram: React.FC = () => {
  ...
  const renderDiagram = useMemo(() => {
    const diagramMap: Record<ViewId, React.ReactNode> = {
      ...
      flow: <FlowDiagram />,
      parallelism: <ParallelismDiagram />,
      affects: <AffectsDiagram />,
      power: <PowerDiagram />,
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap justify-center items-center gap-3 md:gap-4">
          {VIEWS.map((view) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-5 py-3 rounded-full font-bold text-sm transition-all duration-300 border ${
                  isActive
          ...
          <button
            ...
            style={{
              ...
              borderColor:
                mounted && theme === "dark" ? "#f3f4f6" : "#9ca3af",
            }}
            title={
              mounted && theme === "light"
                ? "Switch to Dark Mode"
      ...
      {/* CREDIT FOOTER */}
      <footer className="w-full max-w-screen-xl mx-auto px-6 py-8 flex flex-col md:flex-row justify-between items-center text-sm text-[var(--text-muted)] gap-4">
        <div className="font-medium text-center md:text-left">
          Credit:{" "}
          <span className="font-semibold text-[var(--text-secondary)]">
            Baruch Spinoza
          </span>{" "}
          — <i>Ethica Ordine Geometrico Demonstrata</i> (1677)
        </div>
        <div className="flex items-center gap-1">
          <span>&copy; {currentYear} | Created with passion by </span>
          <a
